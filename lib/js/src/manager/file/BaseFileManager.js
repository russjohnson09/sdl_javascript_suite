...
class BaseFileManager extends BaseSubManager {
	 ...
	 */
    deleteRemoteFileWithName (fileName, listener = null) {
        const rpcRequest = new RpcRequest(
            {
                functionName: 'ListFiles',
                parameters: {
        ...
        (async () => {
            ...
            if (listener !== null) {
                listener.onComplete(rpcResponse.getSuccess());
            }
        })();
    }

    _deleteFileFromLists(filename) {
        let idx1 = this._remoteFiles.indexOf(fileName);
        let idx2 = this._uploadedEphemeralFileNames.indexOf(fileName);
