...
class SdlManager extends EventEmitter {
    ...
    async sendRPCJson(
        ...
        if (bulkData) {
            console.log(`sendRPCJson bulkData`,bulkData);
            rpcMessage.setBulkData(bulkData);
            console.log(`sendRPCJson bulkData`,rpcMessage);
        }
        else {
            console.log(`no bulk data+++++++++++++++++===
            +++++++++++++++++++++++++
            +++++++++++++++++`,rpcMessage.getBulkData())
        }
    ...
    _handleRPCResponse(sdlPacket) {
        ...
        console.log(`_handleRPCResponse`,{version,sessionId,frameType,frames, rawJson, functionId, correlationId, bulkData});
        ...
        emitData.isSuccess = emitData.params && emitData.params.resultCode === 'SUCCESS';


        if (bulkData) {
            if (!(bulkData instanceof Uint8Array)) {
                console.log(`converting bulkdata to Uint8Array`);
